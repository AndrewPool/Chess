...
public class DeciderNode : ITraversable, IEquatable<DeciderNode>
{
    ...
    public DeciderNode(SmartSquare[,] board)
    {
    ...
    public int BestMoveScore()
    {
        ...
        if (Player)// is white
        {
...

            {
                ...
                best = childNode.ScoreForBranch() < best ? childNode.ScoreForBranch() : best;
            }
    ...
    public int ScoreForBranch()
    {
        if (IsLeaf)
        {
            if (board.WhiteWon) return int.MaxValue;
            if (board.BlackWon) return int.MinValue;
            return board.Score;
        }
